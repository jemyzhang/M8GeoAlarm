...
void GeoReminder::updateReminderList(ReminderInfo_ptr r){
...
bool GeoReminder::addReminder(ReminderInfo_ptr r){
	bool ret = false;;
	if(r->CID != 0 && r->LAC != 0){
		if(!updateReminder(r)){
...
bool GeoReminder::updateReminder(ReminderInfo_ptr r){
	bool ret = false;
	if(r->CID != 0 && r->LAC != 0){
		ReminderInfo_ptr pr = getReminder(r->LAC, r->CID);
		if(pr){
			if(pr != r){
				pr->LAC = r->LAC;
				pr->CID = r->CID;
...
ReminderInfo_ptr GeoReminder::deleteReminder(ReminderInfo_ptr r){
	ReminderInfo_ptr r_evnt = getReminder(r->LAC,r->CID);
...
void GeoReminder::clearReminder(){
	...
	for(; i != list_reminder.end(); i++){
		ReminderInfo_ptr r = *i;
		r->name.SetBufferSize(0);
		r->text.SetBufferSize(0);
		r->nextAlarm.SetBufferSize(0);
	}
	list_reminder.clear();
}

ReminderInfo_ptr GeoReminder::getReminder(int lac, int cid){
	list<ReminderInfo_ptr>::iterator i = list_reminder.begin();
	for(; i != list_reminder.end(); i++){
		ReminderInfo_ptr r = *i;
...
ReminderInfo_ptr GeoReminder::getReminderByIndex(int idx){
	if(idx >= list_reminder.size()) return 0;
...
int GeoReminder::getReminderIndex(int lac, int cid){
	if(list_reminder.size() == 0) return -1;
...
//TODO: check reminder
ReminderInfo_ptr GeoReminder::checkReminder(int lac, int cid){
	ReminderInfo_ptr preminder = getReminder(lac,cid);
	if(preminder){
		if(preminder->isEna){
